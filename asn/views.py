            ...
            if qs.asn_status == 1:
                if AsnDetailModel.objects.filter(openid=self.request.auth.openid, asn_code=qs.asn_code,
                    ...
                    for i in range(len(asn_detail_list)):
                        goods_qty_change = stocklist.objects.filter(openid=self.request.auth.openid,
                                                                    goods_code=str(
                                                                        asn_detail_list[i].goods_code)).first()
            ...
            for j in range(len(data['goodsData'])):
                ...
                if int(data['goodsData'][j].get('goods_actual_qty')) == 0:
                    ...
                    if goods_qty_check > 0:
                        ...
                        asn_detail.goods_cost = asn_detail.goods_cost - (
                                    asn_detail.goods_shortage_qty * goods_detail.goods_cost)
                        qs.total_cost = qs.total_cost - (asn_detail.goods_shortage_qty * goods_detail.goods_cost)
                        ...
                        goods_qty_change.sorted_stock = goods_qty_change.sorted_stock + int(
                            data['goodsData'][j].get('goods_actual_qty'))
                    elif goods_qty_check == 0:
                        asn_detail.goods_shortage_qty = 0
                        asn_detail.goods_more_qty = 0
                        goods_qty_change.pre_sort_stock = goods_qty_change.pre_sort_stock - int(
                            data['goodsData'][j].get('goods_actual_qty'))
                        goods_qty_change.sorted_stock = goods_qty_change.sorted_stock + int(
                            data['goodsData'][j].get('goods_actual_qty'))
                    elif goods_qty_check < 0:
                        ...
                        asn_detail.goods_cost = asn_detail.goods_cost + (
                                    asn_detail.goods_more_qty * goods_detail.goods_cost)
        ...
        if qs_list[0].openid != self.request.auth.openid:
            ...
            if 'bin_name' not in data:
                ...
                for i in range(len(data['res_data'])):
                    goods_qty_change = stocklist.objects.filter(openid=self.request.auth.openid,
                                                                goods_code=str(
                                                                    data['res_data'][i]['goods_code'])).first()
...
class FileListDownloadView(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        response['Content-Disposition'] = "attachment; filename='asnlist_{}.csv'".format(
...
class FileDetailDownloadView(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        response['Content-Disposition'] = "attachment; filename='asndetail_{}.csv'".format(
            str(dt.strftime('%Y%m%d%H%M%S%f')))
        return response
